...
import { FaMoneyBillWave, FaCalendarCheck, FaComments, FaTags, FaTshirt } from 'react-icons/fa';
...
export default function StoreHome() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        const messagesQ = query(messagesRef, where('storeId', '==', storeId), where('read', '==', false));
        ...
        itemsSnap.docs.forEach(doc => {
          const data = doc.data();
          if (data.category) catMap[data.category] = (catMap[data.category] || 0) + 1;
          if (data.style) {
  ...
  return (
    <div className="store-home">
      <StoreSidebar currentPage="Home" onLogout={() => auth.signOut().then(() => navigate('/login'))} />
      <div className="analytics-content">
        ...
        <div className="section">
          <h2><FaTags /> Listings Overview by Category</h2>
        ...
        {/* Styles */}
        <div className="section">
          <h2><FaTshirt /> Most Popular Styles</h2>
          <div className="card-grid">
