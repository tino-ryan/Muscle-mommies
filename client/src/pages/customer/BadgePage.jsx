...
const QUEST_API_URL = 'https://witsquest-hjggaxgwfgbeh0gk.brazilsouth-01.azurewebsites.net';
    ...
    <div className="badge-home">
      ...
      <div className="content-container">
        <div className="header">
          ...
          <p>Visit nearby thrift stores and collect badges to unlock exclusive rewards!</p>
        </div>

        {/* Featured ThriftFinder Badge */}
        <div className="featured-badge-section">
          <div className="badge-card featured">
            <img src={thriftBadge.imageUrl} alt={thriftBadge.name} className="badge-image" />
            <div className="badge-info">
              <h2 className="badge-title">{thriftBadge.name}</h2>
              <p className="badge-description">{thriftBadge.description}</p>
              <p className="badge-date">Available since {new Date(thriftBadge.createdAt).toLocaleDateString()}</p>
              <button 
                className="claim-button" 
                onClick={() => window.open('https://witsquest-hjggaxgwfgbeh0gk.brazilsouth-01.azurewebsites.net/', '_blank')}
              >
                Claim Badge & Sign Up
              </button>
              <p className="claim-note">
                To claim this reward, sign up for the Quest app and visit a nearby thrift store!
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="badges-grid">
              {allBadges.slice(0, 6).map((badge) => (
                <div key={badge.id} className="badge-card">
                  ...
                  <p className="badge-date">Available since {new Date(badge.createdAt).toLocaleDateString()}</p>
                </div>
              ))}
            </div>
            {allBadges.length > 6 && (
